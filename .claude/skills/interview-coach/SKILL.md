...
## Project Integration (AI-Agent-New) — приоритет над остальными правилами скилла

Скилл установлен из [noamseg/interview-coach-skill](https://github.com/noamseg/interview-coach-skill)
(MIT) как специализированный тренажёр интервью внутри карьерной системы `career/`.
Правила стыковки:

1. **Источник фактов — `career/profile/profile.md`.** Истории storybank должны
   прослеживаться до профиля; новые факты, всплывшие на тренировках, — сначала
   в профиль (инвариант `career/methodology/01-pipeline.md`). Ничего не выдумывать —
   это совпадает с собственным правилом скилла (Evidence enforcement).
2. **Состояние хранить в `career/interview/coaching_state.md`** (личные данные,
   каталог в .gitignore). Story bank из `career/interview/story-bank.md` и
   coaching_state синхронизировать — истории не должны расходиться.
3. **Разделение труда.** Для резюме — скиллы `resume-build`/`resume-review`;
   разбор вакансии — `vacancy-analysis`; сопроводительные — `cover-letter`;
   вилка и оффер — `offer-negotiation`; стратегия и воронка — `job-search-strategy`.
   Одноимённые команды этого скилла (`resume`, `linkedin`, `decode`, `salary`,
   `negotiate`, `outreach`) использовать как второе мнение, не как замену.
4. **Сильная сторона этого скилла — тренировка**: `mock`, `practice`-дриллы,
   `analyze` (транскрипты), `stories` (коучинг storybank), `hype`, `debrief`,
   `progress`. Скилл `interview-prep` — вход и связка с воронкой; сюда приходить
   за глубокой практикой и скорингом.
5. **Исходы интервью** (вопросы, сигналы, фидбек, статусы) синхронизировать
   с логом откликов `career/search/applications.md`.
6. Язык общения — язык пользователя (русский); внутренние схемы/файлы скилла
   можно вести на английском.
